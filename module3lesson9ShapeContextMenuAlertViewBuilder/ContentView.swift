import SwiftUI


struct ContentView: View {
    var body: some View {
        VStack(spacing: 20) {
            // Оригинал для сравнения
            Text("Оптимизированная версия")
                .font(.headline)
            
            CloudShapeOptimized()
                .fill(Color(red: 0.97, green: 0.84, blue: 0.84))
                .frame(width: 476, height: 221)
                .shadow(color: .black.opacity(0.1), radius: 8, y: 4)
            
            // Уменьшенная версия
            CloudShapeOptimized()
                .fill(
                    LinearGradient(
                        colors: [Color(red: 0.97, green: 0.84, blue: 0.84),
                                Color(red: 0.95, green: 0.80, blue: 0.80)],
                        startPoint: .topLeading,
                        endPoint: .bottomTrailing
                    )
                )
                .frame(width: 300, height: 140)
                .shadow(color: .black.opacity(0.15), radius: 10, y: 5)
        }
        .padding()
    }
}


struct CloudShapeOptimized: Shape {
    func path(in rect: CGRect) -> Path {
        var path = Path()
        
        let svgWidth: CGFloat = 476
        let svgHeight: CGFloat = 221
        
        let scale = min(rect.width / svgWidth, rect.height / svgHeight)
        let dx = (rect.width - svgWidth * scale) / 2
        let dy = (rect.height - svgHeight * scale) / 2
        
        func p(_ x: CGFloat, _ y: CGFloat) -> CGPoint {
            CGPoint(x: x * scale + dx, y: y * scale + dy)
        }
        
        func cp(_ x1: CGFloat, _ y1: CGFloat, _ x2: CGFloat, _ y2: CGFloat, _ x: CGFloat, _ y: CGFloat) {
            path.addCurve(to: p(x, y), control1: p(x1, y1), control2: p(x2, y2))
        }
        
        func qp(_ cx: CGFloat, _ cy: CGFloat, _ x: CGFloat, _ y: CGFloat) {
            path.addQuadCurve(to: p(x, y), control: p(cx, cy))
        }
        
        // Начинаем с левого верхнего угла
        path.move(to: p(2, 25.5))
        
        // Левый верхний скругленный угол
        qp(1, 24, 0, 28)
        qp(0, 30, 0, 191.5)
        
        // Левый нижний угол - плавный подъем
        qp(0, 193, 1, 195)
        qp(2, 197, 3, 199)
        qp(4, 201, 6, 203)
        qp(7, 205, 9, 208)
        qp(11, 210, 14, 212)
        qp(17, 214, 21, 216)
        qp(24, 218, 30, 219.5)
        
        // Верхняя левая часть - плавная кривая
        qp(80, 220.5, 156.5, 220.5)
        qp(159, 220.5, 162, 219.5)
        qp(165, 218.5, 168, 216.5)
        qp(170, 215, 172, 213)
        qp(174, 211, 175.5, 208)
        qp(177, 205, 179, 200)
        qp(181, 196, 184, 193)
        qp(185, 191, 187, 188)
        qp(189, 185, 191, 182)
        qp(193, 179, 197, 177)
        qp(200, 175, 204, 172)
        qp(207, 170, 211, 168)
        qp(214, 166, 220, 164)
        qp(224, 163.5, 251, 163.5)
        
        // Правая верхняя часть - плавная кривая
        qp(257, 163.5, 262, 165)
        qp(266, 167, 270, 169)
        qp(273, 171, 277, 174)
        qp(280, 177, 284, 181)
        qp(287, 184, 290, 188)
        qp(292, 191, 294, 195)
        qp(296, 198, 298, 202)
        qp(299.5, 205, 302, 209)
        qp(304, 212, 307, 214.5)
        qp(309, 216, 312, 217.5)
        qp(314, 218.5, 317, 219.5)
        
        // Правая верхняя часть до края
        qp(380, 220.5, 448, 219.5)
        qp(451, 219, 454, 217.5)
        qp(457, 216, 460, 213)
        qp(463, 210, 466, 206)
        qp(468, 203, 470, 199)
        qp(472, 196, 474, 192)
        qp(475, 191, 476, 191)
        
        // Правая сторона вниз
        path.addLine(to: p(476, 29))
        
        // Правый верхний угол - плавный спуск
        qp(475, 26, 473, 22)
        qp(471, 17, 468, 13)
        qp(466, 10, 463, 7)
        qp(460, 4, 455, 2)
        qp(451, 0.5, 447.5, 0)
        
        // Верхняя часть влево
        path.addLine(to: p(345.5, 0))
        
        // Левый верхний угол - плавный
        qp(341, 0.5, 337, 2)
        qp(332, 4, 327, 8)
        qp(323, 12, 320, 18)
        qp(319, 21, 319, 25)
        path.addLine(to: p(319, 35.5))
        
        // Внутренний изгиб вправо
        qp(318.5, 38, 317, 42)
        qp(316, 45, 314, 49)
        qp(312, 52, 308, 56)
        qp(304, 59, 299, 61)
        qp(295, 61.5, 292, 61.5)
        
        // Нижняя центральная часть
        path.addLine(to: p(181.5, 61.5))
        
        // Левый внутренний изгиб
        qp(178, 61, 175, 59)
        qp(172, 57, 169, 53)
        qp(166, 49, 163, 44)
        qp(161, 40, 159.5, 36)
        
        // Левая сторона вверх
        path.addLine(to: p(159.5, 27.5))
        
        // Левый верхний угол
        qp(159, 24, 157, 19)
        qp(155, 14, 152, 10)
        qp(149, 6, 144, 3)
        qp(140, 1.5, 136.5, 1)
        
        // Низ слева
        path.addLine(to: p(25, 1))
        
        // Замыкающий левый угол
        qp(22, 1.5, 19, 3)
        qp(16, 5, 13, 8)
        qp(11, 10, 9, 13)
        qp(7, 16, 5, 19)
        qp(3.5, 22, 2, 25.5)
        
        path.closeSubpath()
        return path
    }
}

#Preview {
    ContentView()
}
